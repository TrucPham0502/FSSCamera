...
@property (strong, nonatomic)   NSString                    *customizeUrlOutput;
...
- (id)initWithCustomizeUrlOutput:(NSString *)url {
    self = [super init];
    if (self)
    {
        self.customizeUrlOutput = url;
        self.writer = [[GlimpseAssetWriter alloc] initWithCustomizeUrlOutput:url];
    }
    return self;
}
...
- (id)init
{
    ...
    if (self)
    {
        if ([self.customizeUrlOutput isEqual:[NSNull null]] || [self.customizeUrlOutput isEqualToString:@""]) {
            self.writer = [[GlimpseAssetWriter alloc] initWithCustomizeUrlOutput:self.customizeUrlOutput];
        }
        else {
