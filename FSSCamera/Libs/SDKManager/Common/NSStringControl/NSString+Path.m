...
+ (NSString *)getPhotoPath {
    NSString *photosPath = [NSString stringWithFormat:@"%@/%@",[NSString documentsPath],@"Photos"];
    [NSString checkDirectoryExist:photosPath];
    return photosPath;
}
#pragma mark - The total path to save all videos
+ (NSString *)getVideoPaths {
    NSString *videosPath = [[NSString documentsPath] stringByAppendingString:@"/Videos"];
    [NSString checkDirectoryExist:videosPath];
    return videosPath;
}
...
/// @param pathFile pathFile description
+(BOOL)checkFileExist:(NSString*)pathFile{
    NSFileManager *manager = [NSFileManager defaultManager];
...
/// Description remvoe file by path
/// @param pathFile pathFile description
+(BOOL)removeFileByPath:(NSString*)pathFile{
    NSFileManager *manager = [NSFileManager defaultManager];
    //check file exits
    NSError *error = nil;
    if ([NSString checkFileExist:pathFile]){
        return [manager removeItemAtPath:pathFile error:&error];
    }
    return NO;
}
@end
