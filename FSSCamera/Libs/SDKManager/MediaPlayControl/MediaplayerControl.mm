...
#pragma mark - lamnhs truyền đường dẫn để lưu hình ảnh
-(int)snapImage:(NSString *)pictureFilePath{
    return FUN_MediaSnapImage(self.player, [pictureFilePath UTF8String]);
}
...
-(int)startRecordWithPath:(NSString *)movieFilePath{
    return FUN_MediaStartRecord(self.player, [movieFilePath UTF8String]);
}
...
-(void)OnFunSDKResult:(NSNumber *)pParam{
    ...
    //NSLog(@"OnFunSDKResult__%d:",msg->id);
    switch ( msg->id ) {
#pragma mark 收到开始直播结果消息
        case EMSG_START_PLAY:{
            ...
            if ( self.delegate && [self.delegate respondsToSelector:@selector(mediaPlayer:startResultPlayBack:param3:param2:)] ) {
                [self.delegate mediaPlayer:self startResultPlayBack:msg->param1 param3:msg->param3 param2:msg->param2];
            }
        }
            break;
...
#pragma mark 收到查询直播信息结果消息
        case EMSG_ON_PLAY_INFO:{
            ...
            if ([self.delegate respondsToSelector:@selector(mediaPlayer:Param1:Param2:Param3:)]) {
                //回放时间
                [self.delegate mediaPlayer:self Param1:msg->param1 Param2:msg->param2 Param3:msg->param3];
            }
        }
            break;
        case EMSG_ON_PLAY_END: {
            NSString* startStr = [NSString stringWithFormat:@"00:00"];
            if ([self.delegate respondsToSelector:@selector(mediaPlayer:playEndResult:)]) {
                [self.delegate mediaPlayer:self playEndResult:startStr];
            }
