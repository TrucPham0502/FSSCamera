...
- (void)addDeviceByDeviseSerialnumber:(NSString*)serialNumber deviceName:(NSString *)deviceName devType:(int)type seq:(int)seq{
    ...
    devInfo.nPort = 34567;
    FUN_SysAdd_Device(self.msgHandle, &devInfo, SZSTR(@""), SZSTR(@""), seq);
}
...
-(void)startConfigWifiByAPMode:(NSString*)userName userPassword:(NSString*)userPassword ssid:(NSString*)ssid password:(NSString*)password{
    ...
    FUN_DevSetLocalPwd("192.168.10.1",SZSTR(userName), SZSTR(userPassword));
    FUN_DevStartWifiConfigByAPLogin(self.msgHandle, "192.168.10.1", ssid.UTF8String, password.UTF8String, 10000);
}
...
- (void)getDeviceChannel:(NSString *)devMac seq:(int)seq{
    ...
    if (devObject.channelArray.count>0 || nameArr.count >0){
        ...
        //本地已经有保存，后台更新通道名称
        FUN_DevGetChnName(self.msgHandle, [devObject.deviceMac UTF8String], "", "", seq);
        return;
    }
    //2、从服务器获取通道信息，获取成功后刷新
    FUN_DevGetChnName(self.msgHandle, [devMac UTF8String], "", "", seq);
}
...
- (void)logoutDevice:(NSString *)devMac{
    FUN_DevLogout(self.msgHandle, [devMac UTF8String]);
}
...
- (void)OnFunSDKResult:(NSNumber *) pParam {
    @try {
        ...
        MsgContent *msg = (MsgContent *)nAddr;
                NSLog(@"check_msg->id: %d",msg->id);
                NSLog(@"check_msg->param1: %d",msg->param1);
                NSLog(@"check_msg->param2: %d",msg->param2);
                NSLog(@"check_msg->param3: %d",msg->param3);
                NSLog(@"check_msg->szStr: %@",NSSTR(msg->szStr));
                NSLog(@"check_msg->pObject: %@",NSSTR(msg->pObject));
                NSLog(@"check_msg->sep: %d",msg->seq);
        switch (msg->id) {
#pragma mark 添加设备回调
            case EMSG_SYS_ADD_DEVICE:{
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(addDeviceResult:result:seq:)]) {
                    [self.delegate addDeviceResult:NSSTR(msg->pObject) result:msg->param1 seq:msg->seq];
                }
                if (self.delegate &&  [self.delegate respondsToSelector:@selector(addDeviceResultForLoginDevice:result:seq:)]) {
                    [self.delegate addDeviceResultForLoginDevice:NSSTR(msg->pObject) result:msg->param1 seq:msg->seq];
                }
            }
                break;
#pragma mark 获取设备状态回调
            case EMSG_SYS_GET_DEV_STATE:{
                if (self.delegate && [self.delegate respondsToSelector:@selector(getDeviceStateV2:result:)]) {
                    [self.delegate getDeviceStateV2:NSSTR(msg->szStr) result:msg->param1];
                }
...
#pragma mark 获取设备通道回调
            case EMSG_DEV_GET_CHN_NAME:{
                ...
                if (self.delegate &&  [self.delegate respondsToSelector:@selector(getDeviceChannel:result:seq:)]) {
                    [self.delegate getDeviceChannel:devObject.deviceMac result:msg->param1 seq:msg->seq];
                }
                if (self.delegate &&  [self.delegate respondsToSelector:@selector(loginDeviceResult:result:seq:)]) {
                    [self.delegate loginDeviceResult:devObject.deviceMac result:msg->param1 seq:msg->seq];
                }
            }
                break;
...
#pragma mark 删除设备回调
            case EMSG_SYS_DELETE_DEV:{   // 删除设备
                NSMutableArray *deviceArray = [[DeviceControl getInstance] currentDeviceArray];
                if (deviceArray.count <= msg->seq) {
                    //lamnhs xoá device thất bại tiến hành gọi callback thất bại
                    if (self.delegate && [self.delegate respondsToSelector:@selector(deleteDevice:result:)]) {
                        [self.delegate deleteDevice: @"" result:-1];
                    }
                    return;
                }
                DeviceObject *devObject = [deviceArray objectAtIndex:msg->seq];
...
#pragma mark 局域网搜索回调
            case EMSG_DEV_SEARCH_DEVICES:{
                ...
                if (msg->param1 <= 0) {
                    //没有搜索到设备
                    //[MessageUI ShowErrorInt:msg->param1];
                    [self.delegate searchDevice:NULL result:msg->param1];
                    break;
                }
                //[SVProgressHUD dismiss];
...
#pragma mark 判断是否是主账号回调
            ...
            case EMSG_DEV_GET_CONFIG_JSON:{
                if (self.delegate &&  [self.delegate respondsToSelector:@selector(getConfigByJson:jsonResult:)]) {
                    //có lỗi xảy ra
                    NSString *result = [NSString stringWithFormat:@"%s", msg->pObject];
                    [self.delegate getConfigByJson:@"" jsonResult:result];
                }
                break;
            }
            case EMSG_DEV_SET_CONFIG_JSON:{
                if (self.delegate &&  [self.delegate respondsToSelector:@selector(setConfigByJson:)]) {
                    [self.delegate setConfigByJson:msg->param1];
                }
                break;
            }
            default:
    ...
    @catch (NSException *exception) {
        if (self.delegate &&  [self.delegate respondsToSelector:@selector(getDeviceChannel:result:seq:)]) {
            //có lỗi xảy ra
            [self.delegate getDeviceChannel:@"" result:0 seq:-1];
        }
    }
    @finally {
...
- (void)getConfigByJson: (NSString *)serial strConfigName:(NSString *)name{
    FUN_DevGetConfig_Json(self.msgHandle, SZSTR(serial), SZSTR(name), 0, -1);
}

- (void)setConfigByJson: (NSString *)serial strConfigName:(NSString *)name strJsonConfig:(NSString *) config{
    FUN_DevSetConfig_Json(self.msgHandle, SZSTR(serial), SZSTR(name), [config UTF8String], strlen([config UTF8String]), -1);
}
@end
