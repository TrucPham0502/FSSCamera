...
- (void)LinkAlarmPush:(NSString *)deviceMac DeviceName:(NSString *)devName Username:(NSString *)username Password:(NSString *)password{
    MC_LinkDev(self.msgHandle, SZSTR(deviceMac), SZSTR(username), SZSTR(password), 0, SZSTR(devName));
}
//注销设备报警通知
